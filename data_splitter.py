...
def split_file(fileDir, chunkLength, targetDir):
    ...
    for i in range(0, int(newAudio.duration_seconds*1000), chunkLength):
            ...
            fileName = os.path.basename(fileDir).split('.')[0]+"_chunk_{0}.".format(i)+os.path.basename(fileDir).split('.')[1]
...
if __name__ == "__main__":
    ...
    chunkLength = 1000 #Works in milliseconds
    ...
    for file in data.img_paths:
        targetPath = os.path.dirname(file.replace(datasetDir, datasetSplittedDir))
        split_file(file, chunkLength, targetPath)
